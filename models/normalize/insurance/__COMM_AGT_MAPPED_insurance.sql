...
FHFORMAT AS (
    SELECT * 
    FROM {{ ref ('__FHFORMAT_insurance') }}
),
CORRECTED_AGT_SPLITS AS (
    SELECT * 
    FROM {{ ref ('__COMM_WGTS_insurance') }}
),
...
S2A_CASES_WHERE_NUM_AGTS_NOTEQ AS (
    ...
    GROUP BY 1
    HAVING COUNT(DISTINCT FH_SERVICINGAGTCODE) = 1 AND COUNT(DISTINCT COMMISSIONINGAGTCODE) in (2,3)
),
...
SELECT
    ...
    P.FH_PREMIUM
FROM 
    S5_ONE_SERVAGT_MANY_COMM A LEFT JOIN FHFORMAT P
    ON A.POLICYCODE = P.POLICYCODE 
