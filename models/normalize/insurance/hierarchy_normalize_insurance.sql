...
Hierarchy AS(
 SELECT 
        ...
        hps.HIERARCHYPATHNAME,
 		hps.HIERARCHYLEVEL,
 		(SELECT MIN(b.BRANCHID) FROM SANDBOX.EVELYN.H1 b WHERE hps.hierarchypath LIKE CONCAT('%',b.BRANCHID,'%')) AS BRANCHID,
 		LTRIM(RTRIM((SELECT MIN(b.BRANCHNAME) FROM SANDBOX.EVELYN.H1 b WHERE hps.hierarchypath LIKE CONCAT('%',b.BRANCHID,'%')))) AS BRANCHNAME
