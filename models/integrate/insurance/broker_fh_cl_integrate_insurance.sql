...
TAGGED AS (
    SELECT
        ...
        AGENTCODE,
        CASE
            WHEN COUNT(DISTINCT Source) = 2 THEN 'FH + CL Direct'
            WHEN MAX(Source) = 'FH' THEN 'FH Only'
            ELSE 'CL Direct Only'
        END AS Advisor_Type_Tag
    FROM ORIGINAL
    GROUP BY USERDEFINED2, AGENTCODE
),
...
FROM
    ...
    LEFT JOIN TAGGED T ON O.AGENTCODE = T.AGENTCODE AND O.USERDEFINED2 = T.USERDEFINED2
GROUP BY ALL