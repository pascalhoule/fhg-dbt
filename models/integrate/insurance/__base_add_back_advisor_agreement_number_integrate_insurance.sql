...
ALL_DATA AS (
    SELECT 
        ...
        ' ' AS ADVISOR_REPORTING_FIRM_NAME, 
        AC_REGION, 
        AC_MARKET, 
        AC_LOCATION, 
        PRODUCT_KIND, 
        ...
        FYC_AMOUNT
    FROM {{ ref('__base_put_on_one_row_others_integrate_insurance') }}
) , 

REVISED_MAPPING_TABLE AS (
    SELECT
        ADVISOR_AGREEMENT_GROUP_IDENTIFIER,
        CURRENT_CONTRACT_POLICY_NUMBER,
        ADVISOR_PRODUCER_NAME,
        ...
        FIRST_VALUE(ADVISOR_REPORTING_FIRM_NAME) OVER (
            PARTITION BY ADVISOR_AGREEMENT_GROUP_IDENTIFIER,
            CURRENT_CONTRACT_POLICY_NUMBER,
            ADVISOR_PRODUCER_NAME
            ORDER BY
                ADVISOR_REPORTING_FIRM_NAME NULLS LAST
        ) AS ADVISOR_REPORTING_FIRM_NAME,
    FROM
